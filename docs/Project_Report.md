...
| 场景 | 建议 chunk_size | 建议 overlap | 调优逻辑 |
|:-----|:--------------:|:-----------:|:---------|
| **论文/技术文档**（当前） | **500** | **75（15%）** | 段落边界清晰，中等 chunk 保持上下文完整 |
| **代码文档**（含代码块） | **600-800** | **100-120** | 代码块通常较长，大 chunk 避免切断函数/类 |
| **新闻/博客文章** | **300-400** | **45-60** | 段落短小精悍，小 chunk 聚焦具体事实 |
| **法律/合同文档** | **800-1200** | **100-200** | 条款间逻辑紧密，大 chunk 保证上下文不中断 |
| **对话/客服记录** | **200-300** | **30-45** | 单轮对话短，小 chunk 精确匹配查询意图 |
...
| 场景 | 数据量 | 延迟要求 | 建议 ef_search | 建议 ef_construction | 建议 M | 调优逻辑 |
|:-----|:------:|:--------:|:-------------:|:-------------------:|:------|:---------|
| **快速原型 / Demo** | < 1 万 | < 100ms | **30-50** | 100 | 16 | 低 ef_search 保证低延迟，M 保持默认 |
| **生产级 PDF 知识库** | 1-10 万 | < 200ms | **100** | 200 | **16-32** | 增加 ef_search 保证召回，M 适度提升 |
| **高召回场景**（法律/医疗） | < 10 万 | < 500ms | **200-300** | 300-400 | **32-48** | 高 ef_search 和 M 最大化召回，牺牲建库速度 |
| **百万级日志/代码库** | 100 万+ | < 300ms | **100-150** | 200-400 | **16-32** | 数据量大时提升 ef_construction 保证图质量 |
| **实时交互**（ChatBot） | < 1 万 | < 50ms | **20** | 100 | 16 | 极致低延迟，可配合强 Reranker 补偿召回 |
...
| 指标 | RRF=0.5（默认） | RRF=0.4 | **RRF=0.6 🏆** | RRF=0.65 |
|:-----|:--------------:|:-------:|:--------------:|:--------:|
| **Recall@8** | 0.755 | 0.768 | **0.770** 🥇 | 0.758 |
| **HitRate@8** | **0.980** | 0.980 | **0.980** | **0.980** |
| **Faithfulness** | 0.833 | 0.810 | **0.835** | **0.837** |
| **Answer Relevancy** | **0.905** | 0.881 | **0.913** 🥇 | 0.885 |
| **Context Recall** | **0.739** | 0.677 | 0.722 | 0.730 |
| **Context Precision** | **0.492** | 0.151 | 0.473 | 0.348 |
| 检索耗时 | 96.7ms | 99.0ms | **96.1ms** | 157ms |

**结论：RRF=0.6 为最优权重**，Recall@8(0.770)和Answer Relevancy(0.913)均为最高，整体最均衡，检索耗时与默认持平。

### 8.6 Reranker 调优建议

| 场景 | 候选数 | 建议模型 | 总延迟 | 调优逻辑 |
|:-----|:------:|:---------|:------:|:---------|
| **实时对话** | 8-10 | MiniLM-L-4-v2 | ~150ms | 轻量模型 + 少候选，保证 <300ms |
| **高精度问答** | 15-20 | MiniLM-L-6-v2 | ~350ms | 增加候选数和层数提升精度 |
| **离线批量** | 30-50 | bge-reranker-large | ~1-2s | 大模型全量重排，精度优先 |
| **多语言场景** | 15 | bge-reranker-v2-m3 | ~400ms | m3 支持 100+ 语言 |
...
**最终方案：cs500 + ov75 + BM25 (RRF=0.6), top-8**
...
|:-----|:---------:|:------------:|:----------:|:-----:|:-----:|:-----:|
| Recall@8 | 0.758 | **0.770** | 0.758 | 0.725 | 0.735 | 0.775 |
| Faithfulness | 0.814 | **0.835** | 0.815 | 0.809 | 0.810 | 0.766 |
| Answer Relevancy | 0.859 | **0.913** | 0.860 | 0.856 | 0.805 | 0.830 |
| Context Recall | 0.766 | 0.722 | 0.678 | 0.677 | 0.726 | 0.651 |
| Context Precision | 0.439 | 0.473 | **0.575** | 0.562 | 0.593 | 0.320 |
| 检索耗时 | 96.7ms | **96.1ms** | 75.0ms | 108.3ms | 115.8ms | 92.6ms |
| 总 chunks | 2031 | 2042 | ~2090 | 1692 | 1411 | 3383 |
| RRF 权重 | 0.5 | **0.6** | 0.5 | 0.5 | 0.5 | 0.5 |
...
10. **BM25 停用词过滤 + 加权 RRF 融合是必要优化**：英文停用词会导致 BM25 误匹配，向量权重 0.6、BM25 权重 0.4 的加权 RRF 有效抑制了此类噪声
